...
class TestMusicTheory(unittest.TestCase):
    ...
    def test_get_num(self):
        self.assertEqual(get_num("C"), 60)  # default octave is 4 = MIDI 60
        self.assertEqual(get_num("c"), 60)  # default octave is 4 = MIDI 60
        self.assertEqual(get_num("C4"), 60)  # C4 = MIDI 60 (middle C)
        self.assertEqual(get_num("c4"), 60)
        self.assertEqual(get_num("C5"), 72)  # C5 = MIDI 72
        self.assertEqual(get_num("As0"), 22)  # As0 = MIDI 22 in standard MIDI
    ...
    def test_get_note(self):
        self.assertEqual(get_note(60), "C4")  # MIDI 60 = C4 (middle C, standard MIDI)
        self.assertEqual(get_note(48), "C3")  # MIDI 48 = C3
        self.assertEqual(get_note(72), "C5")  # MIDI 72 = C5
        self.assertEqual(get_note(30), "Fs1")
        self.assertEqual(get_note(66), "Fs4")
        self.assertEqual(get_note(50), "D3")
        self.assertEqual(get_note(47), "B2")

    def test_get_scale_num(self):
        self.assertEqual(get_scale_num(72, 'major'), [72, 74, 76, 77, 79, 81, 83])
        self.assertEqual(get_scale_num('C5', 'major'), [72, 74, 76, 77, 79, 81, 83])
        self.assertEqual(get_scale_num(10, 'dorian'), [10, 12, 13, 15, 17, 19, 20])  # Use MIDI number for low notes
        self.assertEqual(get_scale_num(66, 'lydian'), [66, 68, 70, 72, 73, 75, 77])

    def test_get_scale_notes(self):
        self.assertEqual(get_scale_notes(60, 'major'), ["C4", "D4", "E4", "F4", "G4", "A4", "B4"])
        self.assertEqual(get_scale_notes('C4', 'major'), ["C4", "D4", "E4", "F4", "G4", "A4", "B4"])
        self.assertEqual(get_scale_notes(10, 'dorian'), ['As-1', 'C0', 'Cs0', 'Ds0', 'F0', 'G0', 'Gs0'])  # Use MIDI number
        self.assertEqual(get_scale_notes(66, 'lydian'), ['Fs4', 'Gs4', 'As4', 'C5', 'Cs5', 'Ds5', 'F5'])
        with self.assertRaises(ValueError):
    ...
    def test_transpose_note(self):
        self.assertEqual(get_transpose_note('C4'), 'C5')
        self.assertEqual(get_transpose_note('C4', -12), 'C3')
        self.assertEqual(get_transpose_note('C4', 24), 'C6')
        self.assertEqual(get_transpose_note(60), 'C5')

    def test_transpose_num(self):
        self.assertEqual(get_transpose_num('C4'), 72)
        self.assertEqual(get_transpose_num('C4', -12), 48)
        self.assertEqual(get_transpose_num('C4', 24), 84)
